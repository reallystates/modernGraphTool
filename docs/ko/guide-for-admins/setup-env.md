---
title: 배포 방식 고르기
description: "modernGraphTool을 배포하는 네 가지 방법: GitHub Pages(서버 없이 무료 호스팅), CDN(이미
  서버가 있다면 권장), 사전 빌드 릴리스, 소스에서 빌드."
editUrl: true
head: []
template: doc
sidebar:
...
앱 번들을 제외한 데이터·설정·테마는 모든 옵션에서 똑같습니다. 나중에 옵션을 바꾸더라도 데이터베이스를 다시 만들 필요 없이 `index.html`만 바꾸면 됩니다.
...
브라우저에서 편집하는 GitHub Pages를 제외하면, 모든 옵션에 다음이 필요합니다.

1. **프로젝트 파일** — 옵션에 따라 [릴리스](https://github.com/potatosalad775/modernGraphTool/releases)를 받거나 저장소를 클론합니다.
2. **텍스트 에디터** — `config.js`, `theme.css`, `phone_book.json`을 수정할 때 씁니다. [Visual Studio Code](https://code.visualstudio.com/)는 Windows·macOS·Linux에서 무료로 쓸 수 있고, 입력하는 동안 실수를 짚어 줍니다.

[설정 파일 편집기](/config-generator), [phone_book.json 편집기](/phone-book-editor), [테마 생성기](/theme-generator)를 쓰면 이 파일들을 직접 작성하지 않고도 대부분의 편집을 처리할 수 있습니다.